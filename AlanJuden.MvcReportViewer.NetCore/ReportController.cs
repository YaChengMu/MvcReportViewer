...
namespace AlanJuden.MvcReportViewer
{
	public abstract class ReportController : Controller
	{
		...
		public async Task<JsonResult> FindStringInReport(string reportPath, string searchText, int? page = 0)
		{
			var model = this.GetReportViewerModel(Request);
			model.ViewMode = ReportViewModes.View;
			model.ReportPath = reportPath;

            var result = await ReportServiceHelpers.FindStringInReportAsync(model, searchText, page);

            return Json(result.ToInt32());
		}
		...
		public async Task<FileContentResult> ReportImage(string originalPath)
		{
			...
			{
				var imageData = await client.GetByteArrayAsync(originalPath);
